...
Entity
Không cần thêm Entity mới cho bước này.

BLL
Tạo 2 DTO mới trong tầng BLL:

CashbookDetailDTO.cs:1
CashbookSummaryDTO.cs:1
Triển khai thuật toán tồn quỹ đầu kỳ + lũy kế trong ReportService:

Hàm chi tiết theo đúng hướng Opening Balance: ReportService.cs:134
Dòng thêm “SỐ DƯ ĐẦU KỲ”: ReportService.cs:187
Hàm tổng hợp theo danh mục: ReportService.cs:225
GUI
Đồng bộ lại ucCashbookReport để gọi method/DTO mới:
Gọi GetCashbookDetailsAsync: ucCashbookReport.cs:91
Map cột TransactionDate/RunningBalance: ucCashbookReport.cs:130
Map cột TotalTransactions cho chế độ tổng hợp: ucCashbookReport.cs:203
Đã build kiểm tra: Build succeeded (chỉ còn warning cũ của dự án, không có error mới từ phần này).

Documentation Summary

Mục tiêu:
Bổ sung dữ liệu báo cáo cho 2 chế độ Sổ quỹ chi tiết và Tổng hợp, trong đó chế độ chi tiết phải tính đúng tồn quỹ lũy kế bắt đầu từ số dư đầu kỳ.

Task List:

Tạo CashbookDetailDTO và CashbookSummaryDTO trong tầng BLL.
Viết hàm GetCashbookDetailsAsync để tính Opening Balance trước fromDate, sau đó cộng dồn theo thứ tự thời gian.
Viết hàm GetCashbookSummaryAsync để group theo loại giao dịch và danh mục.
Đồng bộ ucCashbookReport sang DTO/property mới để hiển thị đúng cột.
Thu hoạch cho Báo cáo:
Tách rõ dữ liệu chi tiết và dữ liệu tổng hợp giúp code dễ bảo trì hơn.
Tính Running Balance chuẩn nghiệp vụ cần 2 pha: pha quá khứ (trước kỳ) và pha trong kỳ (cộng dồn).
Dòng “SỐ DƯ ĐẦU KỲ” giúp người dùng đọc báo cáo trực quan hơn.
Mô tả:
GetCashbookDetailsAsync lọc theo TenantId, BranchId, Status COMPLETED, IsActive; tính pastIncome và pastExpense trước startDate để ra số dư đầu kỳ.
Sau đó lấy giao dịch trong kỳ, sort tăng dần theo thời gian, cộng/trừ vào currentBalance cho từng dòng và xuất DTO gồm Thu, Chi, RunningBalance, ghi chú, người tạo.
GetCashbookSummaryAsync group theo TransType + CategoryName, trả về số lượng phiếu và tổng tiền theo từng nhóm.
Nhận xét:
BLL đang đúng vai trò xử lý nghiệp vụ và lọc dữ liệu theo scope multi-tenant/branch.
GUI đã nhẹ hơn, chủ yếu bind và format cột.
Dữ liệu xuất Excel cũng rõ ràng hơn nhờ DTO tách theo chế độ.
Khó khăn & Giải pháp:
Khó khăn: entity hiện tại dùng tên trường TransDate, TransType, Description khác với pseudo code TransactionDate, Type, Notes.
Giải pháp: ánh xạ đúng sang DTO mới (TransactionDate, TransactionType, Notes) trong ReportService để giữ chuẩn output mà không sửa entity DB.
Khó khăn: đổi DTO/method có thể làm hỏng màn hình hiện tại.
Giải pháp: đồng bộ ucCashbookReport theo tên property mới và build xác nhận ngay sau khi sửa.
Code

Entity
Không thay đổi Entity ở bước này.

BLL
Không sửa thêm BLL trong lượt này, dùng lại 2 hàm đã có:

ReportService.cs:134
ReportService.cs:225
GUI
Đã refactor toàn bộ code-behind trong ucCashbookReport.cs:
Load màn hình tự set ngày và tự gọi tải dữ liệu: ucCashbookReport.cs:22
Nút xem báo cáo gọi lại load chung: ucCashbookReport.cs:30
Hàm tải dữ liệu dùng thủ thuật đổi Data Source theo radChiTiet/radTongHop, clear lưới cũ trước khi bind mới: ucCashbookReport.cs:74
Format riêng cho sổ quỹ chi tiết, gồm highlight RunningBalance màu xanh đậm: ucCashbookReport.cs:118
Format riêng cho tổng hợp: ucCashbookReport.cs:173
Nút export xuất đúng dataset theo radio đang chọn và tiêu đề file thống nhất: ucCashbookReport.cs:212
Lưu ý: trong project hiện tại context chuẩn là AppDbContext, nên giữ AppDbContext thay vì CashFlowContext để compile đúng.

Kết quả kiểm tra:

Build thành công: Build succeeded with 206 warnings, không có lỗi compile mới từ phần ucCashbookReport.
Documentation Summary

Mục tiêu:
Hoàn thiện code-behind cho màn hình Báo cáo Dòng tiền theo cơ chế chuyển đổi 2 chế độ xem trên cùng DataGridView bằng Radio Button, đồng thời đảm bảo tải dữ liệu bất đồng bộ và xuất Excel theo đúng mode.

Task List:

Refactor hàm Load của UserControl để tự động nạp dữ liệu khi mở màn hình.
Tạo luồng LoadDataAsync dùng chung cho Load ban đầu và nút Xem Báo Cáo.
Áp dụng cơ chế clear lưới và đổi DataSource theo radChiTiet/radTongHop.
Viết FormatDetailGrid và FormatSummaryGrid với cấu hình cột phù hợp từng mode.
Cập nhật nút Export Excel để xuất đúng nguồn dữ liệu hiện tại.
Thu hoạch cho Báo cáo:
Một DataGridView có thể phục vụ nhiều dạng báo cáo nếu tách rõ phần bind và format theo mode.
Tập trung toàn bộ logic nạp dữ liệu vào một hàm dùng chung giúp giảm trùng lặp và dễ bảo trì.
RunningBalance được nhấn mạnh bằng style trực quan giúp người dùng đọc số liệu nhanh hơn.
Mô tả:
Khi mở màn hình hoặc bấm Xem Báo Cáo, hệ thống kiểm tra tenant/date hợp lệ, sau đó gọi service tương ứng theo mode radio.
Nếu mode chi tiết: bind danh sách CashbookDetailDTO và format các cột ngày, loại, danh mục, diễn giải, người lập, các cột tiền.
Nếu mode tổng hợp: bind CashbookSummaryDTO và format nhóm cột loại, danh mục, số lượng phiếu, tổng tiền.
Nút export chọn dataset theo radio hiện tại để tránh xuất sai nội dung.
Nhận xét:
Code-behind hiện đã đúng vai trò tầng GUI: điều phối UI và gọi BLL, không nhồi business logic truy vấn vào giao diện.
Trải nghiệm chuyển đổi mode mượt, ít rối vì không cần mở nhiều màn hình báo cáo tách biệt.
Khả năng mở rộng tốt cho các mode báo cáo mới trong tương lai.
Khó khăn & Giải pháp:
Khó khăn: đồng bộ hành vi giữa Load ban đầu, nút Xem Báo Cáo, và refresh khi context đổi.
Giải pháp: gom toàn bộ vào LoadDataAsync và để các sự kiện chỉ gọi lại hàm này.
Khó khăn: tránh sai dữ liệu khi chuyển mode liên tục.
Giải pháp: luôn clear DataSource/Columns trước khi bind mode mới, sau đó format cột theo đúng DTO.